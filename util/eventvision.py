...
#                 print ('prepare td frame by iterating events took %s seconds', %em_playback_timer.secs)
                ...
                td_img2 = cv2.resize(td_img, (960, 540))
                cv2.imshow('img', td_img2)
...
def read_annotation(filename):
    """Reads in .bin file for annotations"""
    with open(filename, 'rb') as fid:
        rows = np.fromfile(fid, np.int16, 1)[0]
        cols = np.fromfile(fid, np.int16, 1)[0]
        box_contour = np.fromfile(fid, np.int16, rows * cols)
        box_contour = np.reshape(box_contour, (rows, cols))
        box_contour = np.delete(box_contour, 3, axis=1)
        temp = box_contour[0][1]
        box_contour[0][1] = box_contour[1][1]
        box_contour[1][1] = temp
